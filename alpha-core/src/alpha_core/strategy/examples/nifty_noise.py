...
class NiftyNoiseBreakout(Strategy):
    ...
    def on_bar(self, bar: Bar) -> Sequence[Signal]:
        ...
        signals: list[Signal] = []
        if st.side is None and not st.day.entered_today and len(history) == self._cfg.lookback_days:
            ...
            if direction is not None:
                ...
                signals.append(self._signal(bar, direction, "noise-area break", Decimal(1)))
