#!/usr/bin/env python
"""The disciplined G6 sweep: seeded depth-imbalance proposals over the two major cells +
one holdout read per survivor (`docs/research/intraday-edge-survey-2026-07-v2.md`
round-four addendum).

Protocol identical to ``scripts/oi_review.py`` (one in-memory ledger, exhaustive
8-config space per cell via ``max_attempts=500``, holdout only through ``HoldoutGate``
over the pair built in one tested place — here over the research/holdout TICK stores +
the research/holdout DEPTH stores), with two family-specific differences:

- **no ``--cost-scenario`` switch**: the registration is maker-native — post-only entry
  under the #184 strict trade-through fill rule, taker reduce-only exit. That IS the
  deployable execution, so survivors EARN their one-shot holdout read (``auto``); the
  ``skip`` escape hatch stays for floor-thinned windows (#169).
- **one backtester pair PER CELL**: each cell's fold caches ~75M-row 1s arrays (four
  columns); building the pair per cell keeps exactly one symbol resident (~2.5 GB) —
  the 16 GB Mac's one-heavy-job discipline (two OOM kills taught this).

Run AFTER the depth ingest + depth seal (`ingest_binance_depth.py` /
`seal_depth_store.py`)::

    ALPHA_TICK_RESEARCH_ROOT=data_research_ticks ALPHA_TICK_HOLDOUT_ROOT=data_holdout_ticks \\
    ALPHA_DEPTH_RESEARCH_ROOT=data_research_depth ALPHA_DEPTH_HOLDOUT_ROOT=data_holdout_depth \\
        uv run python scripts/depth_review.py
"""
...
from alpha_core.data.depth_store import DepthStore
...
def _sweep_one(
    ...
    *,
    ...
    family = DEPTH_TEMPLATES[template].family
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Seeded G6 depth-imbalance sweep + one holdout read per survivor"
    )
    ap.add_argument("--cells", default=",".join(sorted(DEPTH_CELLS)))
    ...
    bad = [w for w in windows if w not in DEPTH_CELLS]
    if bad:
        raise SystemExit(f"unknown cell(s) {bad}; known: {sorted(DEPTH_CELLS)}")
    ...
    research_depth = DepthStore(Path(os.environ["ALPHA_DEPTH_RESEARCH_ROOT"]))
    holdout_depth = DepthStore(Path(os.environ["ALPHA_DEPTH_HOLDOUT_ROOT"]))
    qa = QuantAnalyst()
    ...
    with ProposalLedger() as ledger:
        ...
        for window in windows:
            ...
            in_sample, holdout_bt = build_depth_backtesters(
            ...
            total_passes += passes
    print(
        f"=== G6[maker-native] verdict: {total_passes}/{total_survivors} survivor(s) "
