#!/usr/bin/env python
"""The disciplined G1 sweep: seeded SIP-ORB proposals over the ONE NIFTY-100 panel cell
(`docs/research/intraday-edge-survey-2026-07-v2.md` §G1 — the strongest external
evidence on the board; survivorship DECLARED in the registration).
...
pair built in one tested place — here over the research/holdout NSE minute BAR stores),
with ONE family-specific default:

- **``--holdout-reads`` defaults to ``skip``**: the NSE|60 holdout window is
  floor-thinned (#144's interval floor — boundary ≈ 2026-06-10) to ~20 sessions, and
  the fold's 14-session RVOL warmup runs INSIDE the holdout (TEST-3), leaving ~6
  readable marks — far too feeble to spend one-shot reads on. Survivors FREEZE; the
  read runs under this same sealed protocol when the window fattens (~Q4-2026), the
  #169/#179 precedent. ``--holdout-reads auto`` exists for that day.
...
`seal_cold_store.py`)::
    ...
    ALPHA_RESEARCH_ROOT=data_research ALPHA_HOLDOUT_ROOT=data_holdout \\
        uv run python scripts/sip_orb_review.py
"""
...
def _sweep_one(
    ...
    *,
    ...
    family = SIP_ORB_TEMPLATES[template].family
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Seeded G1 SIP-ORB portfolio sweep (one panel cell, 4 exhaustive configs)"
    )
    ...
    ap.add_argument(
        ...
        "holdout is floor-thinned to ~20 sessions and the 14-session RVOL warmup runs "
        "inside it (~6 readable marks). 'auto' spends one read per PROMOTE — use only "
        "once the window has fattened (~Q4-2026).",
    )
    args = ap.parse_args()
    ...
    in_sample, holdout_bt = build_sip_orb_backtesters(
    ...
    if not read_holdout:
        print(
            "=== HOLDOUT READS SKIPPED (default): the floor-thinned NSE|60 window minus the "
            "RVOL warmup is too feeble to read — survivors will be FROZEN ==="
        )
    ...
    with ProposalLedger() as ledger:
        strategist = Strategist(
            ...
            templates=SIP_ORB_TEMPLATES,
            max_attempts=500,
        )
        for window in sorted(SIP_ORB_CELLS):
            ...
            total_passes += passes
    print(
        f"=== G1[cash-MIS taker] verdict: {total_passes}/{total_survivors} survivor(s) "
        f"PASS the holdout across {len(SIP_ORB_CELLS)} cell(s) ==="
    )
