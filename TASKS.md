...
| B1b.3 | Workflow A `discovery-cycle`: data prep → strategist → LOOP{ full-rigor backtest (holdout untouched) → quant-analyst → promote? } → survivors | [CC] | ✅ **B1b.3a** (#54) idempotent `proposal_ledger.py` + **B1b.3b(1)** (#56) it's the sole per-cell count source (`TrialLedger` retired, 1a.GATE cell-invariance migrated) + **B1b.3b(2)** (#58) `research/discovery.py` — `run_discovery_cycle` wires propose → backtest → `deflation_inputs` (cumulative count via `trial_index`) → `assess` → survivors. Done-when MET: promotes planted edges, rejects planted overfits; saturation/empty handled; proposal→returns→verdict linkage tested. *(Carried: the real cold-store/engine backtester adapter + cross-run cumulative variance — the backtester is injected today.)* |
...
- **B1b.3c — the real backtester adapter (closes the discovery loop on real data).** `discovery.run_discovery_cycle` takes an *injected* `Backtester` today (tests use a fake). Build the production one: a `Backtester` that, per proposal, `template.build(params)` → runs the **engine** (B0.9 `run_backtest`) on **in-sample bars from the cold store** (B1a.1; holdout untouched, **TEST-3** — carry the holdout-denial test here) with **injected `now`** (bar-time), and reduces fills → a per-bar return series (guaranteeing `>= 2*n_groups` bars). Also: persist a **per-trial score** in the ledger so the deflation **variance** is cumulative across runs (the count already is, via `trial_index`). *(Mac-CLI-only — needs the cold store + engine + real instrument data.)*
- **B1b.4 — `nightly-discovery` schedule + `/knowledge` RAG seeds.** Done-when: the schedule fires a discovery run; RAG answers a strategy-doc query. Then **1b.GATE**.
- **Carried follow-ups (small, before/with 1b.GATE):** the `opening_range_breakout` short-stop test (#48) · extract the triplicated `_sharpe` → a shared metric (#52) · wire the optional cell-level PBO gate into `discovery` (it has the trial×time matrix; #58 NIT).
- **Carried follow-ups (small, before/with 1b.GATE):**
  - **ORB short-stop test (#48 review):** a symmetric short-side stop-loss scenario for `opening_range_breakout` (line 129-130 — the one untested real-money exit branch).
  - **Extract the triplicated `_sharpe` (#52 review):** one shared metric used by `overfitting` / `calibration` / `quant_analyst`.
...
**Phase 1b constrained track COMPLETE end-to-end: B1b.1 strategist (#48/#50) + B1b.2 quant-analyst (#52) + B1b.3 discovery-cycle (#54/#56/#58).** `research/discovery.run_discovery_cycle` turns a search cell into promote/reject/revise decisions + survivors — Done-when MET (promotes planted edges, rejects planted overfits) on the B1a.9 populations. The backtester is **injected** (a fake in tests).

**Next: B1b.3c — the real backtester adapter** (build the strategy → run the engine on cold-store in-sample bars with injected `now` → returns; persist per-trial scores for cumulative variance; Mac-CLI-only) → then **B1b.4** (nightly schedule + `/knowledge` RAG) → **1b.GATE**. **Carried follow-ups:** `opening_range_breakout` short-stop test (#48) · extract triplicated `_sharpe` (#52) · wire the cell-PBO gate into `discovery` (#58). **Conservative-gate note:** the live `dsr.threshold` is **0.92**.
...
**Next: B1a.8 — vectorbt pre-screen + CPCV compute budget (R7)** (cull losers pre-CPCV; night budget respected, queue carried). Introduces `vectorbt` → dev group / research extra only, never the worker kernel. Then **B1a.9 / 1a.GATE** — the **first [You]+[CC] gate since 0.GATE** (Tier-2 rigor error-rate params + population calibration). All under the **PR norm** on the cold store + the locked `alpha_core` engine. *(rigor knobs in `config/rigor.yaml`: cpcv/pbo/dsr/holdout.)*
