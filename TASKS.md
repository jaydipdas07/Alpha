...
- **No clean pure-`[CC]` discovery-pipeline task remains.** What's left needs a direction: (a) run a real nightly on the sealed data (may promote 0 — a Sharpe-~2.4 edge must *exist* in it); (b) start **Phase 2** (cockpit); or (c) a **[You]+[CC]** gate item (below). The B1b.4 live deploy + 1b.GATE are gated.
