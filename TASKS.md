...
- **B1b.3 — Workflow A `discovery-cycle`** (both agents ✅: strategist #48/#50 + quant-analyst #52). Wire the loop: data prep → **strategist proposes** → **full-rigor backtest** (on in-sample data, holdout untouched) → **quant-analyst adjudicates** (`deflation_inputs` → `assess` → promote/reject/revise) → survivors. Done-when: a run produces promote/reject decisions end-to-end. This is where the real backtest returns flow into the quant-analyst (so the cost-sensitivity + look-ahead/survivorship checks that B1b.2 deferred wire in here). Pure-`[CC]` on the research box.
  - **MUST land first (B1b.3 precondition, from the #50 review):** a **persistent proposal-fingerprint store** kept in lock-step with the trial ledger — the strategist's `seen` set must hydrate from it, else the loop re-counts identical trials and corrupts the DSR's per-cell deflation count. The quant-analyst's `deflation_inputs()` consumes that same per-cell trial set.
...
**Next: B1b.3 — Workflow A `discovery-cycle`** (wire strategist → full-rigor backtest → quant-analyst into a loop → survivors). **Precondition (must land first):** a persistent proposal-fingerprint store in lock-step with the trial ledger (#50 review). Then B1b.4 (nightly + RAG) → 1b.GATE. **Carried follow-ups:** the `opening_range_breakout` short-stop test (#48) + extract the triplicated `_sharpe` (#52). **Conservative-gate note:** the live `dsr.threshold` is **0.92**.
