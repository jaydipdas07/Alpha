...
- **M3.0 — Real-data on-ramp (the deferred 1b.GATE demonstration).** Data-vendor ingest adapters — **Tardis / Velo** (crypto derivatives) + the **Indian-leg historical source** (broker historical API or vendor, e.g. TrueData/GlobalDatafeed) — landing into the same cold store via the `data/ingest` seam (the existing Binance/Yahoo adapters are the pattern; data vendors are distinct from execution brokers); re-seal; then the first **real discovery run on operator-provided vendor data** → a genuine survivor passes the full gate end-to-end (feeds Workflow B, M3.7). **[You] provides the data source/keys** (named when active backtesting starts); [CC] builds the adapters + runs discovery. *(Free Binance/Yahoo data carried Phases 0–1; this is where real data is first needed.)*
...
- **Phase 2 is built + DEPLOYED (M2.1–M2.5 ✅, #80–#91).** The cockpit is served at `https://cockpit.apps.lemma.work`, `desk` answers from the pod, the TEST-3 leak is closed. The remaining item — **2.GATE ratification** — is **[You]-gated** (Telegram bot + walk; see 🔒 below). The next *build* phase is **Phase 3** (paper pipeline), but it needs the AWS worker box + operator data/keys ([CC]/[You]) — **no pure-`[CC]` task to start unprompted**; pick it up with [You].
...
**Phases 0 + 1a + 1b are COMPLETE.** Phase 0 = engine bake-off → **Vega lift** (0.GATE, ADR 0001); Phase 1a = the rigor crown-jewel, calibrated + trusted (1a.GATE, ADR 0002 — `dsr.threshold` 0.92, certifies Sharpe ~2.4+ edges); Phase 1b = the AI discovery machine — **strategist → quant-analyst → discovery-cycle → real engine → sealed cold store → nightly**, deployed on the research box (1b.GATE, `docs/adr/0003` closes Phase 1 on machine-validation; the real-edge demonstration is deferred to **M3.0** with operator-provided vendor data). **Phase 2 is COMPLETE + DEPLOYED (M2.1–M2.5 ✅, #80–#91).** The cockpit is served at **`https://cockpit.apps.lemma.work`** (READY, authenticated, live change-stream WS), the `desk` copilot answers grounded in the pod, and the **TEST-3 holdout-metrics leak was caught in review + closed** (`holdout_used`/`holdout_metrics` dropped off the pod `backtests` table — applied live; #91 was human-merged per the cardinal-invariant carve-out). **Only 2.GATE ratification remains — [You]-gated; Telegram DEFERRED (2026-06-28):** the Telegram surface is **wired to the Vega-bot** (ACTIVE/CUSTOM/desk; account created in the web UI — the lemma-0.5.0 CLI account-create is bugged) but [You] deferred the DM-ratification (a bot-setup issue); to finish, resolve it + keep `lemma daemon` running (desk's runtime) + DM the bot. **After 2.GATE → Phase 3** (paper + approval gate + worker + deadman; the M3.0 real-data on-ramp is [You]-provided vendor data). Re-deploy the cockpit via `lemma pods import pod/apps/cockpit`; the demo seed is `pod/seed/seed_cockpit_demo.sh`. The discovery nightly self-fires at 02:00 UTC on the box; real vendor data + the real-edge demonstration come at M3.0 when [You] provides the data.
