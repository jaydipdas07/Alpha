...
- **2.GATE 🔶 pending [You]** — **DEPLOYED + verified (operator-authorized, 2026-06-27):** the cockpit is served at **`https://cockpit.apps.lemma.work`** (READY; renders authenticated; the change-stream **WS is OPEN** — watchChanges is live in production); `desk` answers grounded in the pod (verified: "1 rejected — BTC RSI-Bollinger"); the TEST-3 holdout-column drop is applied live. **Remaining [You]:** create the Telegram bot + connector account, set `account_id` + `is_enabled:true` in `telegram.json`, `lemma pods import pod/surfaces/telegram`; keep `lemma daemon` running (desk's agent runtime); then walk a cockpit scenario + message `desk` on Telegram to ratify. *(Demo data: `pod/seed/seed_cockpit_demo.sh` #83 + `seed_cockpit_demo` heartbeat refresh; a running worker keeps the health strip green.)*
...
- **Phase 2 — 2.GATE ratification ([You]).** The cockpit + `desk` are deployed + verified (#80–#91). To ratify: (1) create the **Telegram bot** + a Lemma connector account → set `account_id` + `is_enabled:true` in `pod/surfaces/telegram/telegram.json` → `lemma pods import pod/surfaces/telegram`; (2) keep **`lemma daemon`** running (desk's agent runtime — CC started it for verification; it stops with the CLI session); (3) walk a cockpit scenario + message `desk` on Telegram. CC can drive (1)/(3) once you create the bot. *(The #91 desk PR was **human-merged** by [You] per the carve-out — it touched TEST-3.)*
