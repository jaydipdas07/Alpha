...
- **B1a.3 — lift Vega rigor** (look-ahead audit + walk-forward + 2× stress + portfolio rigor) into `alpha_core`. The **`backtest/rigor.py` core is already lifted** (B0.9d, ~97% cov via `test_backtest_rigor` + `test_cross_instrument_rigor`); B1a.3 is the **extension** — lift the remaining Vega rigor suites + wire them so "Vega rigor tests green in alpha-core" (the Done-when). Then B1a.4 (CPCV + embargo + PBO) → B1a.5 (DSR + keyed trial ledger) → … → 1a.GATE (the Phase-1a table above). All on the cold store + the locked `alpha_core` engine. *(Follow-up noted in `runner.py`: thread perp funding through the **portfolio** runner before running perps there.)*
...
**Phase 0 COMPLETE (B0.1–B0.10 + 0.GATE ✅, [You] ratified Track B) + namespacing (#27). Phase 1a in progress: B1a.1 ✅ (cold store #28 + ingest #30, 289+21 bars) and B1a.2 ✅ (#32, perp funding R13 → P&L + the kill gate).** 427 tests, ≥94% cov; `main` clean, 0 open PRs.

**Next: B1a.3 — lift Vega rigor** (look-ahead/walk-forward/2× stress + portfolio rigor; the `backtest/rigor.py` core is already lifted — B1a.3 extends it so "Vega rigor tests green in alpha-core"). Then B1a.4 (CPCV/embargo/PBO) → B1a.5 (DSR + keyed trial ledger) → … → **1a.GATE**, all under the **PR norm** on the cold store + the locked `alpha_core` engine. No [You] gate until B1a.9 / 1a.GATE (Tier-2 rigor params).
