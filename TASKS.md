...
- **B1b.4 — `nightly-discovery` schedule + `/knowledge` RAG seeds.** Done-when: the schedule fires a discovery run; RAG answers a strategy-doc query. Then **1b.GATE**. *(The pod schedule/agent wrapper is Mac-CLI + Lemma; the portable discovery run builds anywhere.)*
- **The cold-store `bars_for` adapter** (the one Mac-CLI piece left on the discovery loop): a `BarsFor` that reads **in-sample-only** bars from the no-ACL cold store (B1a.1/B1a.6) for `(market, window)` — and **carries the TEST-3 holdout-denial test** (`EngineBacktester` is a pass-through; isolation lives at this boundary). Needs real instrument data → Mac-CLI.
- **Carried follow-ups (small, before/with 1b.GATE):** persist a **per-trial score** in the ledger so the DSR deflation **variance** is cumulative across runs (the count already is) · the `opening_range_breakout` short-stop test (#48) · extract the triplicated `_sharpe` → a shared metric (#52) · wire the optional cell-PBO gate into `discovery` (#58).
...
**Phase 1b constrained track COMPLETE + real-engine-wired: B1b.1 strategist (#48/#50) + B1b.2 quant-analyst (#52) + B1b.3 discovery-cycle (#54/#56/#58) + B1b.3c the real backtester (#60).** `research/discovery.run_discovery_cycle` turns a search cell into promote/reject/revise decisions + survivors, and now runs **end-to-end on the actual engine** (`EngineBacktester` → `run_backtest` → real returns; a real edge promotes). The only injected seam left is `bars_for` (the cold-store data source).

**Next: B1b.4** (`nightly-discovery` schedule + `/knowledge` RAG) → **1b.GATE**. **Plus the Mac-CLI `bars_for` cold-store adapter** (real in-sample data + the TEST-3 holdout-denial test). **Carried follow-ups:** per-trial score persistence for cumulative deflation variance · `opening_range_breakout` short-stop test (#48) · extract triplicated `_sharpe` (#52) · wire the cell-PBO gate into `discovery` (#58). **Conservative-gate note:** the live `dsr.threshold` is **0.92**.
