...
- **B1a.8 — vectorbt pre-screen + CPCV compute budget (R7).** A coarse vectorbt walk-forward **upstream** of CPCV to cull obvious losers cheaply (so the expensive CPCV/PBO only runs on survivors); a CPCV **compute budget** + parallel pool + **carry-forward** so a night's queue is *carried, not truncated*. **Note:** introduces `vectorbt` — a heavier research dep → goes in the **dev group / a research extra**, never the worker kernel (like duckdb/pyarrow); the parallel pool is process-based. Done-when: losers culled pre-CPCV; the night budget is respected and the queue carried. Then **B1a.9 / 1a.GATE** (the Phase-1a table above — the **first [You]+[CC] gate since 0.GATE**: the Tier-2 error-rate params + population-calibration thresholds). All on the cold store + the locked `alpha_core` engine, under the **PR norm**.
...
**Phase 0 COMPLETE (B0.1–B0.10 + 0.GATE ✅) + namespacing (#27). Phase 1a B1a.1–B1a.7 ✅: B1a.1 (cold store #28 + ingest #30), B1a.2 (#32 perp funding R13), B1a.3 (#34 finalize rigor + portfolio funding), B1a.4 (#35 CPCV+embargo+PBO), B1a.5 (#36 DSR + keyed trial ledger), B1a.6 (#38 no-ACL holdout → TEST-3), B1a.7 (#40 reproducibility ledger).** 477 tests, ≥94% cov (the new rigor/research modules all 100%); `main` clean, 0 open PRs.
