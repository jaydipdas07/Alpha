...
| ID | Task | Owner | Done-when |
|---|---|---|---|
| B1b.1 | `strategist` agent (constrained): parameterize vetted templates (seed from Vega's ~7 example strategies) + whitelisted primitives; economic-rationale prompt; **in-sample only, never sees the holdout**; originality check + trial-ledger increment | [CC] | 🔶 **B1b.1a** (#48) the seed-template substrate lifted — `strategy/registry` + the R7 example strategies (rsi_bollinger/momentum_roc/vwap_reversion/opening_range_breakout) + fuller indicators + configs. **B1b.1b next**: the strategist *logic* (propose a valid config by parameterizing a template + originality check + keyed-ledger increment + holdout isolation). |
...
- **B1b.1b — the `strategist` logic** (B1b.1a substrate ✅ #48). Build the constrained strategist atop the lifted registry/templates: **propose a valid strategy config** by parameterizing a vetted template (within its `from_config` schema) — with an **originality check** (don't re-propose a config already in the cell) + a **keyed-trial-ledger increment** (B1a.5), working **in-sample only / never touching the holdout** (TEST-3, structurally enforced by the B1a.6 no-ACL store). Make the param *proposer* injectable (a seeded sampler for tests; the economic-rationale **LLM proposer** is the pod-agent wrapper, Mac-CLI + Lemma, added later). Done-when: proposes a valid config, holdout untouched. Then **B1b.2** (`quant-analyst`: CPCV/PBO + DSR vs trial count → promote/reject/revise — *rejects a planted overfit, promotes a planted edge*) → **B1b.3** (Workflow A `discovery-cycle`) → **B1b.4** (nightly schedule + `/knowledge` RAG) → **1b.GATE**. All pure-`[CC]` on the research box, atop the trusted Phase-1a gate. *(Open follow-up: a symmetric **short-stop test for `opening_range_breakout`** — line 129-130, the one untested real-money exit branch, flagged in the #48 review.)* *(Environment: the agents/pod are **Mac-CLI-only** — pod + `lemma` CLI + research box.)*
...
**Phase 1b started — B1b.1a ✅ (#48):** the strategist's **seed-template substrate** is lifted (`strategy/registry` + the R7 example strategies + fuller indicators + configs; `vertical_spread`/options + `kill_test`/Phase-3 stay deferred). **534 tests, ≥94% cov.**

**Next: B1b.1b — the `strategist` *logic*** (propose a valid config by parameterizing a template + originality check + keyed-ledger increment + holdout isolation; injectable proposer — the LLM/pod-agent wrapper is Mac-CLI-only, added later). Then B1b.2 (`quant-analyst`) → B1b.3 (Workflow A) → B1b.4 (nightly + RAG) → 1b.GATE. **Conservative-gate note:** the live `dsr.threshold` is **0.92** (certifies Sharpe ~2.4+ edges) — revisit if real candidates show it discards too much genuine edge. **Open follow-up:** the `opening_range_breakout` short-stop test (#48 review NIT).
