...
- **Pure-`[CC]` carried follow-ups: NONE remain.** Done: `_sharpe` → `backtest/metrics.sharpe` (#68); ORB short-stop test + `opening_range_breakout` to 100% (#70). The cell-PBO-into-`discovery` item (#58) was **attempted and reverted** — it is *not* pure-CC (see the [You]+[CC] items below).
