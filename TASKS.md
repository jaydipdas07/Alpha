...
- **Richer cold-store data INGESTED (#73).** `scripts/ingest_cold_store.py` now loads **~18.8k bars across 6 series** (BTC-perp 5m ~7wk + BTC/4-NIFTY-constituent daily ~3y) into the cold store; `config/discovery.yaml` has the 6 cells (all resolve via `ColdStoreBarsFor`). Store root overridable via `ALPHA_COLD_ROOT`. This addresses the **1b.GATE data prerequisite**.
- **NEXT pure-`[CC]`: seal the cold store + point the nightly at the sealed research store.** The ingest writes a **raw/unsealed** store; for a holdout-safe 1b.GATE the dataset must be sealed (`data.holdout.seal_dataset` — research/holdout split, B1a.6) and `run_nightly_discovery_from_config` pointed at the sealed *research* store so discovery never sees the rolled-forward holdout (TEST-3/R6). `seal_dataset` is built + tested — this is **wiring** (a seal step + the nightly's store root). Then a real nightly run on the richer data exercises the gate end-to-end (it may still promote 0 — a Sharpe-~2.4 edge must actually exist in the data).
...
**B1b.4 BUILD COMPLETE — B1b.4a/4b/4c (#64/#66).** The portable nightly run (`research/nightly.py` + `scripts/nightly_discovery.py`) sweeps the universe → real engine → survivors and writes a JSON run-record (`summary()`, shaped to the pod `discovery_runs` table); a research-box **systemd cron** (`deploy/systemd/`) fires it nightly; the `/knowledge` corpus is `docs/knowledge/strategies.md`. Proven on the real cold store (10 cycles fire; 0 promote — correct on thin windows). **Next (two gates):** (1) **[You] deploys** the timer + uploads `/knowledge` (`docs/nightly-discovery.md`) → B1b.4 Done-when met; (2) **1b.GATE** — an AI-proposed strategy passes the full gate end-to-end (richer data now loaded #73; still needs the store **sealed** + an actual edge to exist). **Richer cold-store data ingested (#73 — ~18.8k bars across 6 cells: BTC 5m ~7wk + BTC/4-NIFTY daily ~3y).** **Next pure-`[CC]`: seal the cold store** + point `run_nightly_discovery_from_config` at the sealed *research* store (the TEST-3/holdout wiring — `data.holdout.seal_dataset` is built + tested; this is a seal step + the nightly's store root). Prior pure-CC done: `_sharpe` #68, ORB short-stop #70. **cell-PBO into `discovery` (#58) was attempted + reverted** → it over-rejects robust edge-rich cells (failed the B1b.3 planted-edge Done-when: PBO 0.68 → 0 survivors), so it's **[You]+[CC]** (gate-design + re-calibration), as is the cumulative-variance item. **After sealing, the rest is gated:** (1) [You] live-deploys B1b.4; (2) 1b.GATE — a real nightly on the sealed richer data + an actual edge + [You] ratification. **Nightly wiring note:** point `ColdStoreBarsFor` at the **sealed research** cold store, never a raw ingestion store. **Conservative-gate note:** the live `dsr.threshold` is **0.92**.
