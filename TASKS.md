...
**Done: B0.1/B0.2/B0.3/B0.4/B0.5/B0.6/B0.7 ✅; PR-merge norm (#6); B5 = `TEXT` string-Decimal; 15 pod tables live + seeded; research host up (`3.6.176.133`, alpha-core runs).** The **only** remaining Phase-0 work is the **engine bake-off** — drive under the **PR norm** (build → CI green → sub-agent review → **CC merges** squash): **B0.9** (lift Vega `core`/`execution`/`risk`/`backtest` → `alpha_core`) + **B0.8** (Nautilus + install NautilusTrader) → **B0.10 → 0.GATE** (engine choice → ADR 0001). Build note: when B0.9 lifts the config loader, restore Vega's `_scan_for_secrets`; AWS box ops in `docs/research-host.md`. **Safety-gate edits** (never-do / cardinal invariants / money-live-secrets) → **[You]** merges.
