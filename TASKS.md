...
| B1b.3 | Workflow A `discovery-cycle`: data prep → strategist → LOOP{ full-rigor backtest (holdout untouched) → quant-analyst → promote? } → survivors | [CC] | 🔶 **B1b.3a** (#54) the idempotent `proposal_ledger.py` (a cell's count = #distinct fingerprints → no double-counting; durable; race-free). **B1b.3b(1)** (#56) `ProposalLedger` is now the **sole** per-cell count source — strategist records via it (no in-memory `seen`), `TrialLedger` retired, the 1a.GATE cell-invariance demo migrated. **B1b.3b(2) next**: build the loop itself (data prep → propose → full-rigor backtest → quant-analyst `assess` → survivors). |
...
- **B1b.3b(2) — Workflow A `discovery-cycle`** (the ledger refactor B1b.3b(1) ✅ #56 is done; `ProposalLedger` is the sole count source). Build the loop: **data prep** (in-sample bars from the cold store B1a.1, **holdout untouched** — TEST-3) → **strategist proposes** (`propose(template, market, window)`) → **full-rigor backtest** (build the strategy via `template.build`, run it through the engine → a return series) → **quant-analyst adjudicates** (`deflation_inputs` over the cell's trials → `assess` → promote/reject/revise) → survivors. **Done-when: a run produces promote/reject decisions end-to-end.** The real backtest returns flow in here, so the cost-sensitivity + look-ahead/survivorship checks B1b.2 deferred wire in. **Design Q's to settle:** how to feed the cell's *prior* trial returns to `deflation_inputs` (the ledger stores fingerprints + count, not returns — a per-trial score may need recording after the backtest) and how the loop sources in-sample bars. Then **B1b.4** (nightly + `/knowledge` RAG) → **1b.GATE**.
...
**B1b.3a ✅ (#54)** the idempotent `proposal_ledger.py` + **B1b.3b(1) ✅ (#56)** — `ProposalLedger` is now the SOLE per-cell count source (strategist records via it, no in-memory `seen`; `TrialLedger` retired; the 1a.GATE cell-invariance demo migrated). The desync hazard is closed end-to-end.

**Next: B1b.3b(2)** — build Workflow A `discovery-cycle` (data prep → strategist proposes → full-rigor backtest → quant-analyst `assess` → survivors). Settle: how the cell's prior trial *returns* reach `deflation_inputs` (the ledger stores fingerprints + count, not returns) + how the loop sources in-sample bars (holdout untouched). Then B1b.4 (nightly + RAG) → 1b.GATE. **Carried follow-ups:** the `opening_range_breakout` short-stop test (#48) + extract the triplicated `_sharpe` (#52). **Conservative-gate note:** the live `dsr.threshold` is **0.92**.
