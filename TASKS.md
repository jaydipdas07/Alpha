...
- *(none — **all pure-`[CC]` Phase-1a work is done: B1a.1–B1a.8 ✅**. The only Phase-1a items left are the `[You]`-gated B1a.9 / 1a.GATE below. Next pure-`[CC]` work is **Phase 1b** — AI discovery — which begins after 1a.GATE.)*

### 🟡 Blocked — needs [You]
- **B1a.9 / 1a.GATE — the first `[You]+[CC]` gate since 0.GATE.** **B1a.9** = the **population-calibration harness** (R14): run synthetic **noise / overfit / planted-signal** populations through the full rigor gate (pre-screen → CPCV/PBO → DSR → holdout) and measure **false-promote & false-reject** rates. **`[You]` must set the Tier-2 params first** — the error-rate thresholds the rates must meet (the Phase-1b gate), the DSR search-cell taxonomy, the CPCV embargo size, the synthetic-population definitions. **1a.GATE** then ratifies all four: error-rate thresholds met **+** holdout/ledger unreachable by agents (TEST-3, built in B1a.6) **+** DSR cell-invariance (B1a.5) **+** reproducibility (B1a.7). `[CC]` builds the harness once `[You]` give the Tier-2 thresholds.

### ▶️ Next action for a cold session
**Phase 0 COMPLETE (B0.1–B0.10 + 0.GATE ✅) + namespacing (#27). Phase 1a B1a.1–B1a.8 ✅** — the full rigor crown-jewel: cold store + ingest (#28/#30), perp funding (#32), lifted rigor + portfolio funding (#34), CPCV+embargo+PBO (#35), DSR + keyed trial ledger (#36), no-ACL holdout → TEST-3 (#38), reproducibility ledger (#40), vectorbt pre-screen + CPCV budget (#42/#43). **494 tests, ≥94% cov (the new rigor/research modules all 100%); `main` clean, 0 open PRs.**
