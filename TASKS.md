...
| **0.GATE** | **Engine decision:** pick Track A or B on parity + integration friction + TTM (evidence favors B); record as `docs/adr/0001-engine-bakeoff.md` | [You]+[CC] | 🟡 **recommendation written** — `docs/adr/0001-engine-bakeoff.md` (Status: Proposed) recommends **Track B** (parity tie; B wins friction + TTM decisively). **Pending [You] ratification** → flip ADR to *accepted*, archive `track-a/` + drop `nautilus_trader`, lock `alpha-core`. |
...
- *(Phase-0 build work is complete.)* The next [CC] build is **Phase 1a — B1a.1** (free-data ingest → Parquet/DuckDB cold store), building on the `alpha_core` engine. It opens once **0.GATE** is ratified (below).

### 🟡 Blocked — needs [You]
- **0.GATE — engine ratification** ([You]+[CC]): `docs/adr/0001-engine-bakeoff.md` recommends **Track B** (the lifted Vega engine — parity is a tie; B wins integration friction + time-to-market decisively). **Pending [You]'s call.** On ratify, [CC]: flip the ADR to *accepted*, archive `track-a/` + drop `nautilus_trader`, run the `vega`→`alpha` namespacing PR, and lock `alpha-core` → Phase 1a begins.

### ▶️ Next action for a cold session
**Phase-0 BUILD is done — B0.1–B0.10 ✅:** scaffold/gates/pod-tables/research-host (B0.1–B0.7); the whole **Track-B engine lift + bake-off proof** (B0.9, #14–#22 — runs the B0.7 contract through a deterministic backtest **and** a real Binance-testnet order via `StrategyEngine→OMS(risk)→CcxtAdapter`); the **Track-A** Nautilus shell (B0.8, #24); and **parity + comparison** (B0.10 → `docs/adr/0001`, both tracks net the **same P&L to 8 decimals**). 401 tests, ≥94% cov; `main` clean.

**The one open Phase-0 item is `0.GATE` — [You]'s engine ratification** (see 🟡; recommendation = **Track B**). On ratification, [CC] archives Track A and starts **Phase 1a** (B1a.1 free-data ingest → DuckDB) under the PR norm.
