...
**Next: B1b.4** (`nightly-discovery` schedule + `/knowledge` RAG — the pod schedule/agent wrapper is **Mac-CLI + Lemma**; the portable discovery run builds anywhere) → **1b.GATE** (an AI-proposed strategy passes the full gate end-to-end on the research box). **B1b.4 wiring note:** point `ColdStoreBarsFor` at the **sealed research** cold store, never a raw ingestion store — its in-sample guarantee is inherited from `seal_dataset`, not re-checked in the adapter. **Carried follow-ups:** per-trial cumulative deflation variance · ORB short-stop test (#48) · extract `_sharpe` (#52) · cell-PBO into `discovery` (#58). **Conservative-gate note:** the live `dsr.threshold` is **0.92**.
