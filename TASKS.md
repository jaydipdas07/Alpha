...
| B1b.3 | Workflow A `discovery-cycle`: data prep → strategist → LOOP{ full-rigor backtest (holdout untouched) → quant-analyst → promote? } → survivors | [CC] | 🔶 **B1b.3a** (#54) the precondition `research/proposal_ledger.py` — an **idempotent** fingerprint store (a cell's trial count = #distinct fingerprints, so a re-proposed config can't double-count the DSR deflation; durable → cross-run originality; verified race-free). **B1b.3b next**: wire it into the strategist (drop the in-memory `seen` + `TrialLedger.increment`; make `ProposalLedger` the SOLE per-cell count source) + build the loop (data prep → propose → backtest → assess → survivors). |
...
- **B1b.3b — wire the discovery loop** (the precondition B1b.3a ✅ #54 is in). Two parts:
  1. **Make `ProposalLedger` the sole per-cell count source (#54 review SHOULD-FIX):** rewire the `strategist` to `record()` here (originality via `is_new`, the trial index via `count`) — **drop the in-memory `seen` param + `TrialLedger.increment`** — and retire/rescope `TrialLedger` so two SQLite stores never both hold the rigor-critical trial count (migrate its B1a.5 cell-invariance test onto `ProposalLedger`). A careful refactor of merged code.
  2. **Workflow A `discovery-cycle`:** data prep (in-sample bars from the cold store, **holdout untouched**) → **strategist proposes** → **full-rigor backtest** → **quant-analyst adjudicates** (`deflation_inputs` over the cell's trials → `assess` → promote/reject/revise) → survivors. Done-when: a run produces promote/reject decisions end-to-end. The real backtest returns flow in here, so the cost-sensitivity + look-ahead/survivorship checks B1b.2 deferred wire in. Then **B1b.4** (nightly + `/knowledge` RAG) → **1b.GATE**.
...
**B1b.3a ✅ (#54)** — the idempotent `proposal_ledger.py` (the precondition) is in: a cell's trial count = #distinct fingerprints, so a re-proposed config can't double-count the DSR deflation; durable (cross-run originality); verified race-free.

**Next: B1b.3b** — (1) rewire the strategist to use `ProposalLedger` (drop in-memory `seen` + `TrialLedger.increment`; make it the SOLE per-cell count, retire/rescope `TrialLedger`) + (2) build Workflow A `discovery-cycle` (data prep → propose → full-rigor backtest → quant-analyst → survivors). Then B1b.4 (nightly + RAG) → 1b.GATE. **Carried follow-ups:** the `opening_range_breakout` short-stop test (#48) + extract the triplicated `_sharpe` (#52). **Conservative-gate note:** the live `dsr.threshold` is **0.92**.
