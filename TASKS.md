...
| B1b.4 | `nightly-discovery` schedule + `/knowledge` RAG seeds | [CC] | 🔶 **B1b.4a** (#64) `research/nightly.py` — the **portable nightly run**: `run_nightly_discovery` sweeps every (cell, template) over `config/discovery.yaml` → strategist → real engine (`ColdStoreBarsFor`/`EngineBacktester`) → quant-analyst → survivors; per-cell capital alignment + poison-quarantine + a data-readiness gate (no trial-count burn on empty cells); `scripts/nightly_discovery.py` entry point. Proven on the real cold store (10 cycles fire end-to-end; 0 promote — correct on thin windows). **Remaining: B1b.4b** the Lemma schedule that fires it + **B1b.4c** the `/knowledge` RAG (both Mac-CLI + Lemma). |
...
- **B1b.4b — the Lemma `nightly-discovery` schedule** + **B1b.4c — the `/knowledge` RAG seeds** (B1b.4a, the portable run, is ✅ #64). Done-when: the schedule fires the discovery run; RAG answers a strategy-doc query. Then **1b.GATE**. *(Both are **Mac-CLI + Lemma** and **outward-facing pod authoring** — a self-firing schedule + a RAG agent in the shared Vault; need operator input on cadence + which docs to seed. The schedule fires `scripts/nightly_discovery.py`.)*
- **Carried follow-ups (small, before/with 1b.GATE):** persist a **per-trial score** in the proposal ledger so the DSR deflation **variance** is cumulative across runs (the trial *count* already is, via `trial_index`) · the `opening_range_breakout` short-stop test (#48 — the one untested real-money exit branch, line 129-130) · extract the triplicated `_sharpe` → one shared metric (`overfitting`/`calibration`/`quant_analyst`, #52) · wire the optional cell-PBO gate into `discovery` (#58).
...
**B1b.4a ✅ (#64) — the portable nightly run** (`research/nightly.py` + `scripts/nightly_discovery.py`): sweeps every (cell, template) over the discovery universe → strategist → real engine → quant-analyst → survivors; per-cell capital alignment, poison-quarantine, a data-readiness gate (no trial-count burn). Fires end-to-end on the real cold store (10 cycles; 0 promote — correct on thin windows). **Next: B1b.4b** (the Lemma schedule that fires `scripts/nightly_discovery.py`) + **B1b.4c** (the `/knowledge` RAG) — both **Mac-CLI + Lemma**, **outward-facing pod authoring** (a self-firing schedule + a RAG agent in the shared Vault; need operator input on cadence + which docs to seed) → **1b.GATE** (an AI-proposed strategy passes the full gate end-to-end). **Nightly wiring note:** point `ColdStoreBarsFor` at the **sealed research** cold store, never a raw ingestion store (the in-sample guarantee is inherited from `seal_dataset`). **Carried follow-ups:** per-trial cumulative deflation variance · ORB short-stop test (#48) · extract `_sharpe` (#52) · cell-PBO into `discovery` (#58). **Conservative-gate note:** the live `dsr.threshold` is **0.92**.
