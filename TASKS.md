...
- **B1a.7 — reproducibility ledger.** Record per backtest: strategy version/params, dataset version/hash, `alpha_core` engine version, cost-model version, RNG seed — so a backtest is **bit-for-bit reproducible from its record**. (Foundations exist: the cold store writes byte-stable Parquet (B1a.1); the holdout records `holdout_window_version` (B1a.6); the keyed trial ledger persists per-cell counts (B1a.5) — B1a.7 ties the full provenance into one reproducible record.) Then **B1a.8** (vectorbt pre-screen upstream of CPCV + CPCV compute budget / parallel pool / carry-forward, R7) → **B1a.9 / 1a.GATE** (the Phase-1a table above — the [You]+[CC] Tier-2 error-rate params). All on the cold store + the locked `alpha_core` engine, under the **PR norm**.
...
**Phase 0 COMPLETE (B0.1–B0.10 + 0.GATE ✅, [You] ratified Track B) + namespacing (#27). Phase 1a B1a.1–B1a.6 ✅: B1a.1 (cold store #28 + ingest #30), B1a.2 (#32 perp funding R13), B1a.3 (#34 finalize lifted rigor + portfolio funding), B1a.4 (#35 CPCV+embargo+PBO), B1a.5 (#36 DSR + keyed trial ledger), B1a.6 (#38 no-ACL roll-forward holdout → TEST-3).** 469 tests, ≥94% cov (`overfitting.py`/`dsr.py`/`trial_ledger.py`/`holdout.py` 100%); `main` clean, 0 open PRs.

**Next: B1a.7 — reproducibility ledger** (strategy/dataset/engine/cost versions + RNG seed per backtest → bit-for-bit reproducible). Then B1a.8 (vectorbt pre-screen + CPCV budget, R7) → … → **1a.GATE**, all under the **PR norm** on the cold store + the locked `alpha_core` engine. No [You] gate until B1a.9 / 1a.GATE (Tier-2 rigor params). *(rigor knobs now live in `config/rigor.yaml`: cpcv/pbo/dsr/holdout.)*
