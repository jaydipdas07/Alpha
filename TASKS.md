...
- **B1b.2 — the `quant-analyst` agent** (the strategist B1b.1 ✅ #48/#50). Run a strategist proposal through the **composed, now-trusted rigor gate**: vectorbt pre-screen (B1a.8) → CPCV/PBO (B1a.4) → **DSR vs the cell's trial count** (B1a.5) → look-ahead/survivorship + cost-sensitivity + regime-stability checks → a **promote / reject / revise** verdict. Done-when: **rejects a planted overfit; promotes a planted edge** (reuse the B1a.9 calibration populations as the fixtures). Pure-`[CC]` on the research box, atop the trusted gate (`dsr.threshold` 0.92). Then **B1b.3** (Workflow A `discovery-cycle` — wire strategist→quant-analyst into a loop) → **B1b.4** (nightly schedule + `/knowledge` RAG) → **1b.GATE**.
- **Carried follow-ups (small, do alongside or before 1b.GATE):**
  - **B1b.3 precondition (from the #50 review):** a **persistent proposal-fingerprint store** kept in lock-step with the trial ledger — the strategist's `seen` set MUST be hydrated from it, else the discovery loop re-counts identical trials and corrupts the DSR's per-cell deflation count. Must land before the loop (B1b.3) runs.
  - **ORB short-stop test (from the #48 review):** a symmetric short-side stop-loss scenario for `opening_range_breakout` (line 129-130 — the one untested real-money exit branch).
...
**Next: B1b.2 — the `quant-analyst`** (run a proposal through the composed rigor gate → promote/reject/revise; reject a planted overfit, promote a planted edge). Then B1b.3 (Workflow A) → B1b.4 (nightly + RAG) → 1b.GATE. **Conservative-gate note:** the live `dsr.threshold` is **0.92** (certifies Sharpe ~2.4+ edges). **Carried follow-ups:** a persistent proposal-fingerprint store (B1b.3 precondition, #50 review) + the `opening_range_breakout` short-stop test (#48 review).
