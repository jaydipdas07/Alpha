...
- **Carried follow-up (small, pure-`[CC]`, before/with 1b.GATE):** wire the optional cell-PBO gate into `discovery` (#58 — the loop holds the trial×time matrix `probability_of_backtest_overfitting` wants; it only downgrades PROMOTE→REVISE, so conservative-safe). *(Done: `_sharpe` → `backtest/metrics.sharpe` #68; ORB short-stop test + `opening_range_breakout` to 100% #70.)*
- **Carried follow-up — NOT pure-`[CC]` ([You]+[CC]):** make the DSR deflation **variance** cumulative across runs (persist a per-trial score in the proposal ledger; the trial *count* already is, via `trial_index`). This changes a **calibrated** gate input (the variance feeds the DSR; 1a.GATE calibrated the per-run operating point) → needs re-validation, so it is *not* a free refactor.
