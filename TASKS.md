...
- **B0.8** — Track A (the other half of the bake-off): install NautilusTrader, wrap the B0.7 contract in a Nautilus strategy, wire the Vega-pattern cost model, run a Nautilus backtest + testnet live-paper. *(Full A/B bake-off authorized; **stop-and-ask [You] before starting** — needs NautilusTrader installed.)*

### 🟡 Blocked — needs a prerequisite
- **B0.10 → 0.GATE** — parity bake-off + engine decision (→ `docs/adr/0001`) — **B0.9 ✅**, now needs **B0.8** (Track A) to compare against.

### ▶️ Next action for a cold session
**Done: B0.1–B0.7 ✅; PR-merge norm (#6); B5 = `TEXT`; 15 pod tables + seed; research host up; and ALL of B0.9 (Track B) — B0.9a core (#14), B0.9c risk (#15), B0.9b execution+substrate (#16), B0.9e strategy-engine (#17), B0.9d backtest+portfolio (#18), B0.9f backtest proof (#19) + CcxtAdapter (#21) + Binance-testnet live-paper (#22). The lifted engine runs the B0.7 contract through a deterministic backtest AND placed+filled a real Binance-testnet order via StrategyEngine→OMS(risk)→CcxtAdapter (`docs/bakeoff_b0_9f.md`). 401 tests, ≥94% cov; `main` clean, 0 open PRs.**

**Next: B0.8 — Track A (NautilusTrader shell)**, the other half of the bake-off (see 🟢 above; **stop-and-ask [You]** first — needs NautilusTrader installed). Then **B0.10** (parity oracle + the integration-friction/time-to-market memo across Track A vs B) → **0.GATE** (engine choice → `docs/adr/0001`; evidence favors B). Drive each under the **PR norm** (build → CI green → sub-agent review → **CC merges** squash).
